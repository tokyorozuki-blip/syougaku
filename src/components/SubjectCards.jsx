...
export const SubjectCards = ({ selectedGrade, setSelectedSubject, onStartExercise }) => {
  const handleSelect = (sub) => {
    setSelectedSubject(sub);
    if (onStartExercise) onStartExercise();
  };

  const gradeLabel = !selectedGrade || selectedGrade === 'all' ? '全学年ミックス' : `小${selectedGrade}`;

  return (
    <div>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <h3 className="text-xl font-black flex items-center gap-2 text-slate-800">
          <i className="fa-solid fa-book text-emerald-500"></i> STEP 2: 教科をえらんでスタート！
        </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        >
          ...
          <span className="bg-white/20 text-white text-xs px-2.5 py-1 rounded-full font-bold">各学年500問収録</span>
        ...
        >
          <div className="text-4xl mb-3">🎓</div>
          <span className="bg-yellow-400 text-purple-950 text-xs px-2.5 py-1 rounded-full font-extrabold">県立中受検専用</span>
